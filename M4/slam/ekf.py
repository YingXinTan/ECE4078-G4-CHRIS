...
class EKF:
    ...
    def __init__(self, robot):
        ...
        # flag to free markers' SLAM states
        self.freeze_markers = False
    ...
    def predict(self, raw_drive_meas):
        ...
        # # Step 1: compute predicted state = A @ x = F @ x, and update slam state by calling self.set_state_vector()
        # # Step 2: update self.P with the predicted covariance (refer operate.py to know the order of the function being called, then you will know about self.P)
        # self.set_state_vector(F @ x)
        # self.P = self.predict_covariance(raw_drive_meas)
    ...
    def update(self, measurements):
        ...
        x = self.get_state_vector() # SLAM state

        # TODO: add your codes here to compute the updated x
        # Step 1: compute Kalman gain
        K = self.P @ H.T @ np.linalg.inv( H @ self.P @ H.T + R)
        
        # Step 2: compute the 'corrected' state
        x += K @ (z - z_hat)

        # Step 3: update the state by calling set_state_vector()
        self.set_state_vector(x)
        
        # Step 4: update self.P with the corrected covariance
    ...
    def draw_slam_state(self, res = (320, 500), not_pause=True):
        ...
        for x in np.linspace(start=dx, stop=w-dx, num=cols-1):
            x = int(round(x))
            cv2.line(canvas, (x, 0), (x, h), color=(0,0,0), thickness=1)
        # draw horizontal lines
        for y in np.linspace(start=dy, stop=h-dy, num=rows-1):
            y = int(round(y))
            cv2.line(canvas, (0, y), (w, y), color=(0,0,0), thickness=1)
