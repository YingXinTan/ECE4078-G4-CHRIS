...
def merge_estimations(target_map):
    ...
    ##### CHIA: Instead of taking the first estimation, we take an average over all estimations #####
    if len(redapple_est) > num_per_target:
        x, y = 0, 0
        for i in range(len(redapple_est)):
            x += redapple_est[i][0]
            y += redapple_est[i][1]
        x, y = x/len(redapple_est), y/len(redapple_est)
    ...
    if len(greenapple_est) > num_per_target:
        ...
        x, y = x/len(greenapple_est), y/len(greenapple_est)    
    ...
    if len(orange_est) > num_per_target:
        ...
        for i in range(len(orange_est)):
            x += orange_est[i][0]
            y += orange_est[i][1]
        x, y = x/len(orange_est), y/len(orange_est)
    ...
    if len(mango_est) > num_per_target:
        ...
        for i in range(len(mango_est)):
            x += mango_est[i][0]
            y += mango_est[i][1]
        x, y = x/len(mango_est), y/len(mango_est)
    ...
    if len(capsicum_est) > num_per_target:
        ...
        for i in range(len(capsicum_est)):
            x += capsicum_est[i][0]
            y += capsicum_est[i][1]
        x, y = x/len(capsicum_est), y/len(capsicum_est)
...
#................... Replaced with function versino fo rge same thing ...............
...
#     # print('image_poses :', image_poses)
#     # print('image_poses.keys(): ', image_poses.keys())
