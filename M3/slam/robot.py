...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        nxt_th = th + ang_vel*dt
        R = lin_vel/ang_vel

        # NOTE: Discuss !!!
        if ang_vel == 0:

            DFx[2,0] = -np.sin(th)*lin_vel*dt               # [0,2]
            DFx[2,1] = np.cos(th)*lin_vel*dt                # [1,2]
        else:
            DFx[2,0] = -R*np.cos(th) + R*np.cos(nxt_th)     # [0,2]
            DFx[2,1] = -R*np.sin(th) + R*np.sin(nxt_th)     # [1,2]
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            Jac2[0,1] = (lin_vel/ang_vel) * ( (1/ang_vel) * (np.sin(th) - np.sin(th2)) + dt*np.cos(th2) )
            Jac2[1,1] = (lin_vel/ang_vel) * ( (1/ang_vel) * (-np.cos(th) + np.cos(th2)) + dt*np.sin(th2) )
            Jac2[2,1] = dt
